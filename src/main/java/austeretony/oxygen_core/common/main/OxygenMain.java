...
import austeretony.oxygen_core.common.*;
import austeretony.oxygen_core.common.privilege.*;
...
public class OxygenMain {
    ...
    public void serverStarting(FMLServerStartingEvent event) {
        ...
        for(Group group:groups){
            RoleImpl role=new RoleImpl(group.getName(),10+i, TextFormatting.WHITE);
            role.setPrefix(group.getName());
            i++;
            Collection<PrivilegeRegistry.PrivilegeRegistryEntry> pr= PrivilegeRegistry.getRegisteredPrivileges();
            for(PrivilegeRegistry.PrivilegeRegistryEntry registryEntry:pr){
                String s="oxygen.";
                String[] sarr=registryEntry.name.split(":");
                s+=sarr[0];
                int pos=0;
                String name_old=sarr[1],name_lower=sarr[1].toLowerCase();
                for(int j=1;j<sarr[1].length();j++){
                    if(name_old.charAt(j)!=name_lower.charAt(j)){
                        s+="-";
                        s+=name_lower.substring(pos,j);
                        pos=j;
                    }
                }
                s+="-"+name_lower.substring(pos);
                System.out.println(s);
                for(Node n:group.getNodes()){
                    if(n.getKey().contains("prefix")){
                        String[]sl=n.getKey().split("\\.");
                        String prefix="";
                        for(int k=2;k<sl.length;k++){
                            if(k!=2)prefix+=".";
                            sl[k]=sl[k].replaceAll("\\\\","");
                            prefix+=sl[k];
                        }
                        role.setNameColor(OxygenUtils.formattingFromAmpCode(prefix.substring(0,2)));
                        prefix=prefix.substring(2);
                        role.setName(prefix);
                        continue;
                    }
                    if(n.getKey().equals(s)) {
                        if (registryEntry.type == EnumValueType.BOOLEAN) {
                            if (n.getValue() == true){
                                System.out.println(s+" "+true);
                                role.addPrivilege(new PrivilegeImpl(new TypedValueBoolean(true), registryEntry.id));
                            }else{
                                System.out.println(s+" "+false);
                                role.addPrivilege(new PrivilegeImpl(new TypedValueBoolean(false), registryEntry.id));
                            }
                        } else {
                            if (registryEntry.type == EnumValueType.INT) {
                                if (n.getValue()==true){
                                    System.out.println(s+" "+n.getContexts().getValues("value").toArray()[0]);
                                    role.addPrivilege(new PrivilegeImpl(new TypedValueInteger(Integer.parseInt((String) n.getContexts().getValues("value").toArray()[0])), registryEntry.id));
                                }else{
                                    System.out.println(s+" "+0);
                                    role.addPrivilege(new PrivilegeImpl(new TypedValueInteger(0), registryEntry.id));
                                }
                            } else {
                                if (registryEntry.type == EnumValueType.LONG) {
                                    if (n.getValue()==true){
                                        System.out.println(s+" "+n.getContexts().getValues("value").toArray()[0]);
                                        role.addPrivilege(new PrivilegeImpl(new TypedValueLong(Long.parseLong((String) n.getContexts().getValues("value").toArray()[0])), registryEntry.id));
                                    }else{
                                        System.out.println(s+" "+0);
                                        role.addPrivilege(new PrivilegeImpl(new TypedValueLong(0), registryEntry.id));
                                    }
                                }
                            }
                        }
            ...
            //if(OxygenManagerServer.instance().getPrivilegesContainer().getRole(i+10)==null) {
            OxygenManagerServer.instance().getPrivilegesContainer().addRole(role);
            OxygenManagerServer.instance().getPrivilegesContainer().markChanged();
            OxygenManagerServer.instance().getPrivilegesContainer().compressPrivilegesData();
            //}
        }
